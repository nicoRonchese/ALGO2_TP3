...
void Objetivos::sorteo_objetivos(){
 ...
 for (int i = 0; i < TOTAL_OBJETIVOS_JUGADOR; i++){
  objetivos[i] = new objetivo;
  objetivos[i]->tipo_objetivo = objetivos_preparados[i];
  objetivos[i]->cumplido = false;
  if (objetivos[i]->tipo_objetivo == CANSADO)
   objetivos[i]->cantidad = 50;
  else
   objetivos[i]->cantidad = 0;
 }
}
